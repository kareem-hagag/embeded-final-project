...
{
    /* 1. Set Fast PWM Mode (WGM00 = 1, WGM01 = 1) */
    SET_BIT(TCCR0, WGM00);
    SET_BIT(TCCR0, WGM01);

    /* 2. Set Non-Inverting Mode (COM01 = 1, COM00 = 0) */
    /* Clear OC0 on compare match, set OC0 at BOTTOM */
    SET_BIT(TCCR0, COM01);
    CLR_BIT(TCCR0, COM00);

    /* 3. Set Prescaler (e.g., Prescaler 64) */
    SET_BIT(TCCR0, CS00);
    SET_BIT(TCCR0, CS01);
    CLR_BIT(TCCR0, CS02);

    /* 4. Calculate OCR0 Value from Duty Cycle (0 to 100%) */
...
{
    /* 1. Set Phase Correct PWM Mode (WGM00 = 1, WGM01 = 0) */
    SET_BIT(TCCR0, WGM00);
    CLR_BIT(TCCR0, WGM01);
